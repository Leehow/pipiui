...
```bash
./make-app.sh                 # release → build/PipiUI.app（默认交付路径）
...
`build/PipiUI.app/Contents/MacOS/PipiUI` **必须** 新于本次变更的 sources。否则不算完成。
...
| 可双击 / 给用户打开的 App | `./make-app.sh`（release 打包，**唯一权威产物**） |
...
- 声称完成前：有命令级证据（构建/打包输出 + 时间戳），禁止口头「应该好了」。

---

**权威产物路径：** `build/PipiUI.app`  
