...
/** Compact provider logo tile — deterministic, no network images.
...
export function ProviderLogo({ provider, modelId, size = 14 }: { provider: string; modelId?: string; size?: number }) {
  const { brand, paths, glyph, color } = providerLogoInfo(provider, modelId)
  return (
    <span
    ...
    >
      {paths && paths.length ? (
        <svg viewBox="0 0 24 24" style={{ width: '78%', height: '78%', display: 'block', margin: '11%' }} fill="#fff" aria-hidden="true">
