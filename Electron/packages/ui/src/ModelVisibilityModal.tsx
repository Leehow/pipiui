...
export function ModelVisibilityModal({ host, visibility, vision, updates, current, onModelState, onRequestUpdate, onClose, initialView = 'manage', projectId, sessionId }: {
  ...
  const [tab, setTab] = useState<'general' | 'models' | 'extensions' | 'updates' | 'secrets'>('models')
    ...
    <div className="model-modal-backdrop" data-testid="model-modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}>
      <section className="model-modal" role="dialog" aria-modal="true" aria-label="设置" data-testid="model-modal">
        <header>
          ...
          <p>{tab === 'general' ? '主线模型不支持图片时，用指定识图模型识别图片后交给文字模型。' : tab === 'updates' ? '比较内置 Pi、Cua Driver 和托管运行时组件的本机与最新版本。' : tab === 'extensions' ? '把外部 MCP 或 Pi 扩展加进来。点添加，复制一句话到主界面即可。' : tab === 'secrets' ? '全局加密密钥库。列表只有名称，明文只注入已挂载会话的 Worker 环境。' : view === 'manage' ? '左侧勾选控制底栏快捷模型菜单是否显示；当前模型在快捷菜单中保底可见。' : '登录 pi 支持的 provider 后，其模型目录会自动出现。'}</p>
          ...
          <div className="model-modal-tabs" role="tablist" aria-label="设置分类">
            ...
            <button
              ...
              className={`model-modal-tab${tab === 'secrets' ? ' active' : ''}`}
              data-testid="model-tab-secrets"
              onClick={() => { setExtensionsAddOpen(false); setTab('secrets') }}
            >密钥库</button>
          </div>
        </header>
        <div className="model-modal-body" data-testid="model-modal-body">
          {tab === 'general'
            ...
            : tab === 'secrets'
              ? <SecretVaultPane host={host} sessionId={sessionId} />
            : tab === 'extensions'
