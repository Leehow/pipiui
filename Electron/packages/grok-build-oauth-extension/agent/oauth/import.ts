...
export async function importFromGlobalGrok(opts: { confirm: boolean; homedirOverride?: string }): Promise<{ imported: boolean; reason?: string }> {
  if (!opts.confirm) {
  ...
  if (!parsed || typeof parsed !== "object") return { imported: false, reason: "invalid auth shape" };
  ...
  if (!key) return { imported: false, reason: "no token in global auth" };
