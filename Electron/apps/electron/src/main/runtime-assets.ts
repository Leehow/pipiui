...
export interface AssetLookup {
  ...
  arch?: string
  /** `app.getPath('userData')`. Darwin copies Node/CUA out of the rejected Dev bundle here. */
  userData?: string
}
...
/** Prefer a raw Mach-O. Nested Helper.app execs make syspolicyd re-verify the parent App. */
export function resolveCuaDriverLaunchPath(
  configuredPath: string,
  platform: NodeJS.Platform = process.platform,
): string {
  if (!configuredPath) return configuredPath
  if (platform === 'darwin' && configuredPath.includes(`${CUA_DRIVER_HELPER_APP}/`)) {
    const raw = join(dirname(configuredPath), '..', '..', '..', basename(configuredPath))
    if (existsSync(raw)) return raw
  }
  return configuredPath
}

/** Copy a sealed-in-app Mach-O to userData and ad-hoc sign it so Gatekeeper does not scan PipiUI Electron.app. */
export function materializeDetachedDarwinHost(source: string, destDir: string | undefined, name: string): string {
  if (!destDir || !source || !existsSync(source)) return source
  mkdirSync(destDir, { recursive: true })
  const dest = join(destDir, name)
  try {
    const srcStat = statSync(source)
    const destStat = existsSync(dest) ? statSync(dest) : undefined
    if (!destStat || destStat.size !== srcStat.size || destStat.mtimeMs < srcStat.mtimeMs) {
      copyFileSync(source, dest)
      spawnSync('codesign', ['--sign', '-', '--force', '--timestamp=none', dest], { stdio: 'ignore' })
    }
    try { accessSync(dest, constants.X_OK) } catch { /* copied file may not be +x in tests */ }
    return dest
  } catch {
    return source
  }
...
function electronNodeHost(_platform: NodeJS.Platform, execPath = process.execPath): string {
  return execPath
}
...
function resolveEmbeddedPi(
  ...
  const bundledNode = requireFile(root, manifest.nodeExecutable, 'Node executable')
  const node = platform === 'darwin'
    ? materializeDetachedDarwinHost(bundledNode, userData ? join(userData, 'detached-hosts') : undefined, 'node')
    : bundledNode
...
export function resolveRuntimeAssets(lookup: AssetLookup): ResolvedAssets {
  ...
  const detachDir = lookup.userData ? join(lookup.userData, 'detached-hosts') : undefined
  const resolveCua = (configured: string): string => {
    const raw = resolveCuaDriverLaunchPath(configured, platform)
    return platform === 'darwin' ? materializeDetachedDarwinHost(raw, detachDir, 'cua-driver') : raw
  }
  if (lookup.packaged) {
    ...
    return {
      sourceRoot,
      cuaDriver: resolveCua(env.PIPIUI_CUA_DRIVER_PATH ?? join(root, 'cua-driver', driverName)),
      ...resolveEmbeddedPi(join(root, 'pipiui-embedded'), env, platform, arch, lookup.userData)
    }
  }
  ...
  const embedded = shouldUseEmbedded ? resolveEmbeddedPi(embeddedRoot, env, platform, arch, lookup.userData) : {}
  return {
    sourceRoot: env.PIPIUI_RUNTIME_SOURCE_ROOT ?? fileIfPresent(join(electronRoot, 'resources', 'runtime')),
    cuaDriver: resolveCua(env.PIPIUI_CUA_DRIVER_PATH ?? join(electronRoot, '.cua-driver', driverName)),
