...
export const HERMES_MEMORY_PACKAGE_VERSION = "0.9.6";
...
function requireFunction<T extends Function>(value: unknown, name: string): T {
  if (typeof value !== "function") {
    throw new HermesMemoryAdapterError("shape-incompatible", `pi-hermes-memory 0.9.6 is missing ${name}.`);
  }
  return value as T;
}
...
function requireString(value: unknown, name: string): string {
  if (typeof value !== "string" || !value.trim()) {
    throw new HermesMemoryAdapterError("shape-incompatible", `pi-hermes-memory 0.9.6 has an invalid ${name}.`);
  }
  return value;
}
...
function loadedHermesConfig(bridge: HermesBridge): UnknownRecord {
  ...
  if (!isRecord(config)) {
    throw new HermesMemoryAdapterError("shape-incompatible", "pi-hermes-memory 0.9.6 returned an invalid configuration shape.");
  }
  return config;
}
...
/**
 * Real main-only adapter for pi-hermes-memory 0.9.6. Its internals are checked
