...
## Deterministic Memory Learning Loop eval

Run locally without a network, model, developer-home directory, or App launch:

```sh
npm run eval:memory
```

The versioned `eval/memory-learning-loop.v1.json` corpus covers deterministic recall, abstention, conflict/tombstone, scope/capability, and promotion/forgetting contracts. The CLI emits a human summary plus one machine-readable JSON report. Runtime metrics are local-only versioned DTOs with no query/body/token/text fields.
