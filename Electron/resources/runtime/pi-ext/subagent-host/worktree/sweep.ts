...
export type LeftoverWorktreeCleanupClaimV1 = {
	releaseOnEnd: boolean;
	release(): void | Promise<void>;
};

export type LeftoverWorktreeCleanupClaimResultV1 =
	| { ok: true; claim: LeftoverWorktreeCleanupClaimV1 }
	| { ok: false; status: LeftoverWorktreeLeaseStatusV1; message: string };
...
export type LeftoverWorktreeLeaseHooksV1 = {
	...
	/** Hold exclusive lease ownership from inspect through worktree removal. */
	claimCleanup?(agentId: string): LeftoverWorktreeCleanupClaimResultV1 | Promise<LeftoverWorktreeCleanupClaimResultV1>;
};
...
let leftoverSweepHooks: { afterLeasePrecheck?: (agentId: string) => void | Promise<void> } | undefined;

/** Test-only: runs after the unlocked live precheck and before claimCleanup. */
export function setLeftoverSweepHooksForTests(
	hooks?: { afterLeasePrecheck?: (agentId: string) => void | Promise<void> },
): void {
	leftoverSweepHooks = hooks;
}
...
async function sweepRegisteredWorktree(input: {
	...
	await leftoverSweepHooks?.afterLeasePrecheck?.(agentId);

	if (!lease?.claimCleanup) {
		return item("kept-blocked", "sweep refuses to delete without a cleanup claim");
	}

	let claimed;
	try {
		claimed = await lease.claimCleanup(agentId);
	} catch (error) {
		return item("kept-blocked", `cleanup claim failed: ${error instanceof Error ? error.message : String(error)}`);
	}
	if (!claimed.ok) {
		return item(
			claimed.status === "live" ? "kept-live" : "kept-blocked",
			claimed.status === "live"
				? "another live lease still owns the worktree"
				: `could not claim the worktree lease: ${claimed.message}`,
		);
	}
	if (!claimed.claim.releaseOnEnd) {
		try { await claimed.claim.release(); } catch { /* reused live lease */ }
		return item("kept-live", "live lease still owns the worktree");
	}

	try {
		if (lease.inspect) {
			const status = await lease.inspect(agentId);
			if (status === "blocked") return item("kept-blocked", "lease cannot be inspected safely after claim");
		}
		...
		try {
			...
			/* claim release is best-effort after Git settled */
		}
	}
}
