    ...
    const component = {
        render(width) {
            ...
            if (descriptions.length > 0) {
                rowContent.push("", ...descriptions.flatMap((description) => wrapTextWithAnsi(options.theme.fg("dim", `  ${safeMenuText(description)}`), safeWidth)));
            }
            const content = options.screen.enableSearch
                ...
                : rowContent;
            return renderFrame(options.screen.title, options.screen.lines ?? [], content, options.screen.hint ?? "back", width, options, row?.kind === "action" ? "select" : "toggle");
        },
        invalidate() {
            if (options.screen.enableSearch)
