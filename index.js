...
function assemblePiSpawn(input) {
  ...
  if (enabled(f, "philosophy"))
    ext(args, p.philosophy);
  if (enabled(f, "generateImage"))
    ext(args, p.media);
  if (enabled(f, "git"))
    ext(args, p.git);
  if (enabled(f, "reload"))
  ...
  if (enabled(f, "webSearch")) {
    ...
    if (p.webSearch)
      env.PIPIUI_WEB_ACCESS_EXT = p.webSearch;
  }
  if (enabled(f, "arxivFetch")) {
    ext(args, p.arxivFetchPackage);
    if (p.arxivFetchPackage)
      env.PIPIUI_ARXIV_EXT = p.arxivFetchPackage;
  }
  if (enabled(f, "mcp"))
    ext(args, p.mcp);
  if (enabled(f, "skillLoader"))
    ext(args, p.skillLoader);
  ...
  if (enabled(f, "codexServerTools"))
    ext(args, p.codexServerTools);
  if (enabled(f, "claudeServerTools"))
    ext(args, p.claudeServerTools);
  ...
  if (enabled(f, "subagent") && p.subagentDir) {
    ...
    if (p.agentsDir)
      env.PIPIUI_AGENTS_DIR = p.agentsDir;
    ...
    if (input.mainModelId)
      env.PIPIUI_MAIN_MODEL = input.mainModelId;
  }
  if (!input.bridgePort)
    return { args, env };
  ...
  if (enabled(f, "browser"))
    ext(args, p.webview);
  if (enabled(f, "philosophy"))
    ext(args, p.planRuntime);
...
function resolveSpawnPaths(runtimeRoot = defaultRuntimeRoot()) {
  ...
  return { philosophy: declaredEntrypoint(path.join(runtimeRoot, "pi-philosophy")), media: fileIfPresent(runtimeRoot, "pipiui-media.ts"), git: fileIfPresent(runtimeRoot, "pipiui-git.ts"), reload: fileIfPresent(runtimeRoot, "pipiui-reload.ts"), skillLoader: fileIfPresent(runtimeRoot, "pipiui-skillloader.ts"), planRuntime: fileIfPresent(runtimeRoot, "pipiui-plan-runtime.ts"), searchScope: fileIfPresent(runtimeRoot, "pipiui-search-scope.ts"), codexServerTools: fileIfPresent(runtimeRoot, "pipiui-codex-server-tools.ts"), claudeServerTools: fileIfPresent(runtimeRoot, "pipiui-claude-server-tools.ts"), computerUse: fileIfPresent(runtimeRoot, "pipiui-computer-use.ts"), webview: fileIfPresent(runtimeRoot, "pipiui-electron-webview.ts"), subagentDir: fileIfPresent(ext2, "subagent"), agentsDir: fileIfPresent(ext2, "agents"), memoryBroker: packageIfPresent(ext2, "packages", "memory-broker"), arxivFetchPackage: packageIfPresent(ext2, "packages", "arxiv-fetch"), webSearch: managed("pi-web-access", "0.20.0"), mcp: managed("pi-mcp-extension", "1.5.0") };
}
function installElectronBrowserExtension(runtimeRoot) {
  const sourcePath = path.join(runtimeRoot, "pipiui-webview.ts");
  let source;
  try {
  ...
  if (!source.includes('name: "browser"') || !source.includes("async function bridge("))
    return void 0;
...
class LeaseManager {
  ...
  expired(record) {
    return !Number.isFinite(Date.parse(record.expiresAt)) || Date.parse(record.expiresAt) <= this.now();
  }
  same(record) {
    return record.instanceId === this.instanceId;
  }
  startHeartbeat() {
    if (!this.timer)
  ...
  async query() {
    ...
    if (this.same(holder))
      return { sessionId: this.sessionId, writable: true, holder };
  ...
  releaseSync() {
    if (!this.owned || !fs.existsSync(this.leasePath))
      return;
    try {
...
function githubBrowserURL(remote) {
  ...
  if (path2.toLowerCase().endsWith(".git"))
...
async function probeGit(cwd) {
  ...
  if (inside?.trim() !== "true")
    return { ...EMPTY_GIT_STATUS, localBranches: [] };
...
class HostBridge {
  ...
  register(sessionId) {
    for (const [capability2, owner] of this.sessions)
      if (owner === sessionId)
        this.sessions.delete(capability2);
    const capability = crypto$1.randomBytes(32).toString("base64url");
    this.sessions.set(capability, sessionId);
    return capability;
  }
  unregister(sessionId) {
    for (const [capability, owner] of this.sessions)
      if (owner === sessionId)
  ...
  sessionFor(capability) {
    if (typeof capability !== "string" || !capability)
      return void 0;
    for (const [known, sessionId] of this.sessions)
      if (safeEqual(known, capability))
        return sessionId;
    return void 0;
  }
  route(request, response) {
    ...
    request.on("error", () => {
      if (!response.writableEnded)
        this.reply(response, 400, { ok: false, error: "request aborted" });
    });
  }
  async dispatch(body, response) {
    const sessionId = body && typeof body === "object" ? this.sessionFor(body.sessionCapability) : void 0;
    if (!sessionId || body.schemaVersion !== 1)
      return this.reply(response, 403, { ok: false, error: "unauthorized bridge capability" });
    const event = body.event;
    if (!event || typeof event !== "object")
      return this.reply(response, 400, { ok: false, error: "missing event" });
    try {
      if (body.action === "agent_event")
        this.handlers.onAgentEvent(event, sessionId);
      else if (body.action === "plan_event")
        this.handlers.onPlanEvent?.(event, sessionId);
      else if (body.action === "browser_action")
        return this.reply(response, 200, await (this.handlers.onBrowserAction?.(event, sessionId) ?? Promise.resolve({ ok: false, error: "browser host unavailable" })));
      else
        return this.reply(response, 400, { ok: false, error: `unsupported action ${String(body.action)}` });
    } catch (error) {
...
const DEFAULT_FEATURES = Object.freeze({ philosophy: true, subagent: true, memoryBroker: true, git: true, generateImage: true, reload: true, webSearch: true, arxivFetch: true, mcp: true, skillLoader: true, codexServerTools: true, claudeServerTools: true, browser: true });
class ProviderLoginSession {
  ...
  prompt(p) {
    ...
    if (p.type === "select" && p.options?.length === 1)
      return Promise.resolve(p.options[0].id);
...
class ProviderAuthBackend {
  ...
  async listProviders() {
    ...
    if (!Array.isArray(registry))
      throw new Error("无法读取 pi provider 目录：返回了无效 registry");
    const result = [];
    const diagnostics = [];
  ...
  async continueLogin(loginId, input) {
    ...
    if (!session)
      return { kind: "failed", error: "登录会话不存在或已结束" };
    if (input !== void 0 && input !== "") {
      const answered = await session.answer(input);
      if (!answered && session.cancelled)
...
class SessionMessageQueue {
  ...
  enqueue(sessionId, input) {
    ...
    if (!text2.trim() && attachments.length === 0)
      throw new Error("cannot enqueue an empty message");
  ...
  updateMessage(sessionId, id, input) {
    ...
    if (current.state === "sending")
      throw new Error(`message ${id} is already sending`);
    ...
    if (!text2.trim() && attachments.length === 0)
      throw new Error("cannot update to an empty message");
  ...
  removeMessage(sessionId, id) {
    ...
    if (index < 0)
      throw new Error(`unknown queued message ${id}`);
    if (session.items[index].state === "sending")
      throw new Error(`message ${id} is already sending`);
  ...
  async steerMessage(sessionId, id) {
    ...
    if (session.dispatching)
      throw new Error(`session ${sessionId} is already delivering a message`);
  ...
  retryMessage(sessionId, id) {
    ...
    if (current.state !== "failed")
      throw new Error(`message ${id} is ${current.state}; only failed messages can be retried`);
  ...
  async drain(sessionId) {
    const session = this.state(sessionId);
    if (session.dispatching || session.turnActive)
      return;
    ...
    if (session.dispatchPromise === acknowledgement)
      session.dispatchPromise = void 0;
    if (!delivered && !session.turnActive && !session.dispatching)
      await this.drain(sessionId);
  }
  /**
  ...
  async send(sessionId, item, behavior) {
    ...
    try {
      ...
      if (index >= 0 && session.items[index]?.id === item.id)
        session.items[index] = freeze({ ...sending, state: "failed", error: errorMessage(error) });
      this.changed(sessionId);
      return false;
    } finally {
      session.dispatching = false;
...
function attachment(value) {
  if (!isRecord$1(value) || typeof value.dataBase64 !== "string" || typeof value.mimeType !== "string")
    return void 0;
  return copy(value);
}
function restoreItem(value, sessionId) {
  if (!isRecord$1(value) || typeof value.id !== "string" || value.id.length === 0 || typeof value.text !== "string" || !Array.isArray(value.attachments) || typeof value.createdAt !== "number" || !Number.isFinite(value.createdAt))
    return void 0;
  const attachments = value.attachments.map(attachment);
  if (attachments.some((item) => !item))
    return void 0;
...
class FileQueueStore {
  ...
  async load(sessionId) {
    ...
    if (!isRecord$1(raw) || raw.version !== 1 || raw.sessionId !== sessionId || !Array.isArray(raw.items))
      return [];
...
function quotaProviderFor(provider) {
  if (provider.toLowerCase().includes("relay"))
    return void 0;
  const p = provider.toLowerCase();
  if (p === "xai" || p.includes("grok"))
    return "grok";
  if (p.includes("zai") || p.includes("zhipu") || p.includes("bigmodel"))
    return "glm";
  ...
  if (p.includes("qoder"))
    return "qoder";
  if (p.includes("qwen-token-plan"))
    return "qwenTokenPlan";
  if (p.includes("opencode-go"))
    return "opencodeGo";
  return void 0;
}
...
class QuotaStore {
  ...
  async snapshot(modelProvider, force = false) {
    ...
    if (cached && !force && now - cached.fetchedAt < QUOTA_STALE_AFTER_MS)
...
async function readHistory(path2, offset = 0, limit = 500) {
  ...
  if (stat.size > SESSION_MANAGER_MAX_BYTES) {
    console.warn(`[pipi-backend] SessionManager skipped for ${path2}: ${stat.size} bytes exceeds bounded history limit`);
  ...
  try {
    ...
    console.warn(`[pipi-backend] SessionManager fallback for ${path2}: ${error instanceof Error ? error.message : String(error)}`);
    return readHistoryFallback(path2, offset, limit);
  }
}
...
function supportsImagesFor(modelId, provider, input) {
  ...
  if (visionHints.some((h) => p.includes(h) || m.includes(h)))
    return true;
  if (m.includes("vl") || p.includes("vl"))
    return true;
  return true;
}
...
class PiHostBackend {
  ...
  constructor(options = {}) {
    ...
    if (this.features.browser)
      installElectronBrowserExtension(this.runtimeRoot);
  ...
  async close() {
    ...
    await Promise.all([...this.leases.values()].map((lease) => lease.release().catch(() => void 0)));
    await Promise.all([...this.queueWrites.values()].map((write) => write.catch(() => void 0)));
    this.leases.clear();
  ...
  persistQueue(id, items) {
    ...
    void write.catch((error) => console.warn(`[pipi-backend] queue persistence failed for ${id}: ${error instanceof Error ? error.message : String(error)}`));
  }
  async loadQueue(id) {
    let task = this.queueLoads.get(id);
    if (!task) {
      task = this.queueStore.load(id).then((items) => this.queue.restoreQueue(id, items)).catch((error) => {
        console.warn(`[pipi-backend] queue restore failed for ${id}: ${error instanceof Error ? error.message : String(error)}`);
  ...
  async locate(id) {
    for (const item of await this.index())
      if (item.header.id === id)
        return this.confirmSessionMeta(item);
  ...
  async confirmSessionMeta(meta) {
    try {
      ...
      console.warn(`[pipi-backend] SessionManager metadata fallback for ${meta.path}: ${error instanceof Error ? error.message : String(error)}`);
      return meta;
    }
  }
  project(path$1) {
  ...
  async loadConfiguredModels() {
    ...
    this.modelsLoaded = (async () => {
      ...
      try {
        settings = JSON.parse(await fs.promises.readFile(path.join(this.agentDir, "settings.json"), "utf8"));
      } catch {
      }
      try {
        catalog = JSON.parse(await fs.promises.readFile(path.join(this.agentDir, "models.json"), "utf8"));
      } catch {
      }
      const configured = [];
        ...
        for (const model of config.models ?? [])
          if (typeof model?.id === "string")
            configured.push({
              ...
              supportsImages: supportsImagesFor(model.id, model.provider ?? provider, model.input)
            });
      }
      this.configuredModels = configured;
      this.models = [...configured];
      const preferred = configured.find((model) => model.provider === settings.defaultProvider && model.id === settings.defaultModel) ?? configured.find((model) => model.id === settings.defaultModel) ?? configured[0];
      if (preferred)
        this.modelState = {
          model: preferred,
  ...
  async handle(method, params) {
    switch (method) {
      case "listProjects":
        return (await this.loadProjectPaths()).map((path2) => this.project(path2));
      ...
      case "newSession":
        return this.newSession(params[0], params[1]);
      case "resumeSession": {
      ...
      case "getSessionHistory": {
        ...
        return readHistory(session.path, Number(params[1] ?? 0), Number(params[2] ?? 500));
      }
      ...
      case "sendPrompt":
        return this.enqueueMessage(params[0], params[1], params[2]);
      case "listQueue":
      ...
      case "updateQueuedMessage": {
        ...
        await this.loadQueue(params[0]);
        return this.queue.updateMessage(params[0], params[1], input);
      }
      case "removeQueuedMessage":
        await this.loadQueue(params[0]);
        return this.queue.removeMessage(params[0], params[1]);
      case "promoteQueuedMessage":
        await this.loadQueue(params[0]);
        return this.queue.promoteMessage(params[0], params[1]);
      case "steerQueuedMessage":
        await this.loadQueue(params[0]);
        return this.queue.steerMessage(params[0], params[1]);
      case "retryQueuedMessage":
        await this.loadQueue(params[0]);
        return this.queue.retryMessage(params[0], params[1]);
      case "stop":
        return this.command(params[0], { type: "abort" }).then(() => void 0);
      case "queueFollowUp":
      ...
      case "beginProviderLogin":
        return {
          loginId: this.auth.beginLogin(params[0], params[1])
        };
      case "continueProviderLogin":
        return this.auth.continueLogin(params[0], params[1]);
      ...
      case "setModel":
        ...
        return this.setModel(params[0], params[1], params[2]);
      case "setThinkingLevel":
        return this.setThinking(params[0], params[1]);
      ...
      case "listAgents": {
        ...
        return [...this.agents.values()].filter((agent) => !sessionId || agent.sessionId === sessionId);
      }
      case "abortAgent":
      ...
      case "gitCheckout":
        return checkoutBranch(await this.projectPath(params[0]), params[1]);
      case "capabilities":
        return {
          computerUse: false,
  ...
  async newSession(projectId, name) {
    ...
    const path$1 = path.join(dir, `${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}_${id}.jsonl`);
    const header = {
      type: "session",
      version: 3,
      id,
  ...
  async ensure(id) {
    ...
    const child = this.proc(this.pi, ["--mode", "rpc", ...output.args], {
      cwd: found.header.cwd,
      env: withToolPath({ ...sanitizeEnvironment(this.env), ...output.env }, this.pi),
  ...
  rpcEvent(live, e) {
    ...
    if (e.type === "agent_start") {
      ...
      if (d.type?.startsWith("toolcall_"))
        this.stream({
          type: "tool_call",
          sessionId: id,
          toolCallId: d.toolCall?.id ?? d.id ?? `content-${d.contentIndex ?? 0}`,
          name: d.toolCall?.name ?? "tool",
          delta: d.delta
        });
  ...
  command(id, body) {
    return this.ensure(id).then((live) => new Promise((resolve, reject) => {
  ...
  async refreshState(live) {
    try {
      ...
      const model = state.model ? {
        ...
        supportsImages: supportsImagesFor(state.model.id, state.model.provider, state.model.input)
  ...
  async readSettings() {
    ...
    try {
      ...
      if (error?.code === "ENOENT")
        return {};
      throw new Error(`无法读取 PipiUI 设置：${error instanceof Error ? error.message : String(error)}`);
    }
    if (!isRecord(value))
      throw new Error("PipiUI 设置必须是 object");
    return value;
  }
  ...
  async addProject(value) {
    ...
    if (!paths.includes(value))
      await this.saveProjectPaths([...paths, value]);
  ...
  async removeProject(projectId) {
    ...
    if (!paths.some((path2) => dirId(path2) === projectId))
      throw new Error(`unknown project ${projectId}`);
    await this.saveProjectPaths(paths.filter((path2) => dirId(path2) !== projectId));
  }
  ...
  async mergeRuntimeModels(includeCurrent = true) {
    const merged = new Map(this.configuredModels.map((model) => [
    ...
    try {
      ...
      const additions = [...available].map((model) => this.toRuntimeModel(model)).sort((a, b) => a.provider.localeCompare(b.provider) || a.id.localeCompare(b.id));
  ...
  async prepareImageMessage(live, text2, attachments) {
    ...
    lines.push("");
    if (paths.length === 1)
      lines.push(`Attached image file: ${paths[0]}`);
    else {
      lines.push("Attached image files:");
      for (const p of paths)
        lines.push(`- ${p}`);
    }
    lines.push("(Images are also embedded multimodally; prefer viewing them directly. If you use the read tool, use the paths above — do not invent paths like /home/workdir/attachments/.)");
    return lines.join("\n");
  }
  ...
  async prompt(id, prompt, follow, attachments) {
    ...
    await this.dispatchQueuedMessage(id, { text: prompt, attachments: [] }, "follow_up");
  ...
  async setConfiguredModel(provider, modelId) {
    await this.loadModelCatalog();
    const model = this.models.find((item) => item.provider === provider && item.id === modelId);
    if (!model)
  ...
  async getSessionStats(sessionId) {
    ...
    if (!id)
      throw new Error("no active session; pass an explicit sessionId");
    const live = await this.ensure(id);
  ...
  */
  async worktreeCommand(id, operation) {
    ...
    throw new Error(`manual worktree ${operation} is not implemented in this host yet: retained worktree ${current.path ?? id} on branch ${current.branch ?? "(unknown)"} is untouched. Successful workers are merged automatically; resolve this one from the Swift app or with git directly.`);
  }
  /**
...
;globalThis.__pipiBrowserDOM.dispatch(${JSON.stringify(request)})`;
  ...
  async setViewBounds(bounds) {
    ...
    if (this.active && this.shownTabId !== this.active.id) void this.show(this.active, "restore");
  }
  subscribe(listener) {
    this.listeners.add(listener);
    return () => this.listeners.delete(listener);
  ...
  async show(tab, kind, allowHidden = false) {
    ...
    const view = this.ensureView();
    view.setBounds(visible ? this.bounds : toolOffscreenBounds);
    view.setVisible?.(true);
...
function createWindow(browser) {
  ...
  browser.attachToWindow((view) => window.contentView.addChildView(view));
  window.on("closed", () => browser.detachWindow());
...
if (electron.app) {
  electron.app.whenReady().then(() => {
    ...
    const piBackend = createPiHostBackend({ browserAction: (request) => browser.toolAction(request) });
