...
/// Pi extension: on-demand skill loading.
///
/// Pi renders every model-invocable skill's full description into the system prompt, and
/// hides `disable-model-invocation` skills from it entirely. Both halves are wrong for this
/// app: the descriptions are a per-turn tax on a menu the session rarely uses, and a skill
/// the model cannot see is one it can never reach on its own — the heavy spec/ticket
/// workflows were unreachable without the user typing `/skill:name`.
///
/// This replaces the rendered catalog with a names-only index and two tools: `skill_search`
/// for descriptions, `skill_load` for the body. Same trade as deferred tool schemas — the
/// name is the discovery hook, the payload arrives only when something actually needs it.
///
/// Deliberately a separate extension rather than more logic inside the subagent extension:
/// dispatched workers must keep running with no skills at all, and this file only ever
/// loads in the main session.
enum SkillLoaderExtension {
    static func install(into dir: URL) -> String? {
        let file = dir.appendingPathComponent("pipiui-skillloader.ts")
        do {
...
// On-demand skill loading: names-only index in the prompt, bodies behind two tools.
