...
enum ComputerRuntimeBudget {
    // Compatibility name retained for existing callers. These are transport
    // and event-loop limits, not user-facing policy or a control lease budget.
    static let maximumApprovalSeconds: TimeInterval = 10
    static let maximumExecutionSeconds: TimeInterval = 30
    static let maximumEstimatedSeconds: TimeInterval = 28
    static let finalCaptureAllowance: TimeInterval = 4
    static let maximumPauseSeconds: TimeInterval = 10
    static let maximumTypedUTF16Units = 65_536
        ...
        guard estimate <= maximumEstimatedSeconds else {
            throw ComputerRequestError.invalidRequest(
                "estimated execution time \(String(format: "%.1f", estimate))s "
                    + "exceeds the \(Int(maximumEstimatedSeconds))s transport-safe request limit"
            )
        }
    }
}
