...
// Structured git tools for the agent: status / diff / log / show. No shell strings — argv only.
...
function buildSnapshot(cwd: string): string {
  ...
  lines.push(
    "Tool: git (actions: status | diff | log | show | help). Use it instead of raw shell git for these read operations.",
  );
  return lines.join("\n");
}
...
const GIT_HELP = [
  ...
  "log {n?, path?}           Recent commits via `git log --oneline --decorate -n <n>`.",
  "                          n defaults to 20 (max 100). path limits to one pathspec and must",
  "                          not start with '-'.",
  "show {ref?, full?}        Commit details via `git show --stat <ref>` (ref defaults to HEAD).",
  "                          full=true drops --stat and shows the complete patch. Output is",
  "                          capped at ~80KB with a truncation note.",
  "help                      This text.",
  "",
  "Read-only only — use bash for write operations (commit/add/checkout/etc).",
].join("\n");
...
function gitLog(params: { n?: number; path?: string }) {
  const cwd = process.cwd();
  const inside = runGit(["rev-parse", "--is-inside-work-tree"], cwd);
  if (!inside.ok || inside.stdout.trim() !== "true") {
    return text("not a git repository", true);
  ...
  let n = typeof params.n === "number" && Number.isFinite(params.n) ? Math.floor(params.n) : 20;
  if (n < 1) n = 1;
  if (n > 100) n = 100;
  ...
  const args = ["log", "--oneline", "--decorate", "-n", String(n)];
  if (pathSpec) {
    args.push("--", pathSpec);
  }
  ...
  if (!result.ok) {
    return text(`git log failed: ${result.stderr || result.code}`, true);
  }
  ...
  const { text: out, truncated } = truncateBytes(raw, DIFF_MAX_BYTES);
  const header = truncated
    ? `log (n=${n}${pathSpec ? `; path=${pathSpec}` : ""}; truncated at ${DIFF_MAX_BYTES} bytes)\n\n`
    : `log (n=${n}${pathSpec ? `; path=${pathSpec}` : ""})\n\n`;
  return text(header + out);
}

function gitShow(params: { ref?: string; full?: boolean }) {
  const cwd = process.cwd();
  const inside = runGit(["rev-parse", "--is-inside-work-tree"], cwd);
  if (!inside.ok || inside.stdout.trim() !== "true") {
    return text("not a git repository", true);
  ...
  // Reject ref that looks like a flag to avoid option injection.
  if (ref.startsWith("-")) {
    return text("invalid ref: must not start with '-'", true);
  }

  const full = !!params.full;
  const args = full ? ["show", ref] : ["show", "--stat", ref];

  const result = runGit(args, cwd);
  if (!result.ok) {
    return text(`git show failed: ${result.stderr || result.code}`, true);
  }

  const raw = result.stdout;
  if (!raw.trim()) {
    return text("(empty show)");
  }

  const { text: out, truncated } = truncateBytes(raw, DIFF_MAX_BYTES);
  const mode = full ? "full" : "stat";
  const header = truncated
    ? `show (${mode}; ref=${ref}; truncated at ${DIFF_MAX_BYTES} bytes)\n\n`
    : `show (${mode}; ref=${ref})\n\n`;
  return text(header + out);
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "Git",
    description:
      "Structured read-only git for this project. actions: status, diff{staged?, path?}, " +
      "log{n?, path?}, show{ref?, full?}, help. " +
      'Use this instead of raw shell git for these reads. Call action:"help" for details.',
    promptSnippet: "Structured git status / diff / log / show",
    promptGuidelines: [
      "Use the git tool instead of raw shell git for status, diff, log, and show.",
      "If a diff or show comes back truncated, narrow it with path=/ref= or full=false instead of re-running it.",
    ],
    parameters: Type.Object({
      action: Type.String({ description: "status | diff | log | show | help" }),
      staged: Type.Optional(Type.Boolean()),
      path: Type.Optional(Type.String()),
      n: Type.Optional(Type.Number({ description: "log: commit count (default 20, max 100)" })),
      ref: Type.Optional(Type.String({ description: "show: commit-ish (default HEAD)" })),
      full: Type.Optional(Type.Boolean({ description: "show: true = full patch, false = --stat" })),
    }),
