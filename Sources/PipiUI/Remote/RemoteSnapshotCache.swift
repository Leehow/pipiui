...
final class RemoteSnapshotCache {
    ...
    private struct Entry {
        ...
        let streamingMessage: RemoteTranscriptMessageDTO?
        let revision: UInt64
    ...
    func resolve(
        ...
        if canReuseStreaming, let previous {
            streamingMessage = previous.streamingMessage
        } else if let streamingItem = input.streamingItem {
            streamingMessage = RemoteTranscriptNormalizer.normalizedMessages(
                [streamingItem],
        ...
        var messages = finalizedMessages
