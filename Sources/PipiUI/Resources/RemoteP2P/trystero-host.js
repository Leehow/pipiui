...
(() => {
  ...
  var createHandshakeManager = ({ onPeerHandshake, onHandshakeError, handshakeTimeoutMs, sendHandshakeData, sendHandshakeReady, onActivate, onFailure }) => {
    ...
    return {
      ...
      receiveHandshakeData: (data, id, metadata) => {
        ...
        const pending2 = state.handshakeWaiters.shift();
        if (pending2) {
          pending2.resolve(payload);
          return;
        }
    ...
    const makeInternalAction = (type, options = {}) => {
      ...
      actions[type] = {
        ...
        setOnComplete: (f) => {
          ...
          const pending2 = pendingActionPayloads[type];
          if (pending2?.length) {
            delete pendingActionPayloads[type];
            pending2.forEach(({ payload, peerId, metadata }) => f(payload, peerId, metadata));
          }
        },
        setOnProgress: (f) => {
  ...
  var room_default = (onPeer, onPeerLeave, onSelfLeave, { onPeerHandshake, onHandshakeError, handshakeTimeoutMs = defaultHandshakeTimeoutMs, isPassive = false } = {}) => {
    ...
    if (isBrowser) unregisterBeforeUnloadCleanup = registerBeforeUnloadCleanup(() => leave2().catch(noOp));
    return {
      makeAction,
      leave: leave2,
      ping: async (id) => {
  ...
  var SharedPeerManager = class {
      ...
      const proxy = {
        ...
        replaceTrack: (oldTrack, newTrack) => {
          ...
          if (oldEntry) {
            ...
            oldEntry.rooms.forEach((room2) => nextEntry.rooms.add(room2));
    ...
    dispatchData(shared, data) {
      ...
      if (!binding) {
        const pending2 = shared.pendingDataByToken.get(decoded.roomToken) ?? [];
        pending2.push(decoded.payload);
        shared.pendingDataByToken.set(decoded.roomToken, pending2);
        return;
      }
  ...
  // node_modules/@trystero-p2p/ws-relay/dist/index.mjs
  var relayManager = createRelayManager((client) => client.socket);
  var msgHandlers = relayManager.scoped();
  var isRecord = (value) => typeof value === "object" && value !== null && !Array.isArray(value);
  var isStrategyMessage = (value) => typeof value === "string" || isRecord(value);
  var parseServerMessage = (data) => {
    try {
  ...
  var joinRoom = topic_strategy_default({
    init: (config) => config.relayConfig.urls.map((url) => {
      const client = relayManager.register(url, () => makeSocket(url, (data) => {
        ...
        if (!msg) return;
        msgHandlers.forRelay(client)[msg.topic]?.forEach((handler) => handler(msg.topic, msg.payload));
      }));
      return client.ready;
    }),
    subscribeTopic: (client, topic, onMessage) => {
      const handlers = msgHandlers.forRelay(client);
      const topicHandler = (topic2, data) => void onMessage(topic2, data);
      const wasEmpty = !handlers[topic];
      (handlers[topic] ??= /* @__PURE__ */ new Set()).add(topicHandler);
      if (wasEmpty) client.send(toJson(subscribe(topic)));
      return () => {
  ...
  // src/trystero-peer-gate.ts
  var OneTimePeerGate = class {
    #acceptedPeerID = null;
    #consumed = false;
    reserveBrowser(peerID, remote) {
      const value = remote;
  ...
  // src/trystero-host.ts
  var room = null;
  var generation = "";
  var peerGate = new OneTimePeerGate();
  ...
  function nativeRequest(command, body) {
  ...
  async function start(config) {
    ...
    const next = joinRoom({
      ...
      ...["127.0.0.1", "localhost"].includes(new URL(config.signalURL).hostname) ? { _test_only_mdnsHostFallbackToLoopback: true } : {}
    }, config.roomID, {
    ...
    room = next;
    const action = next.makeAction("pipiui-command-v1", { kind: "request" });
    action.onRequest = async (data, context) => {
      if (!peerGate.accepts(context.peerId) || !data || typeof data.command !== "string" || !data.body || typeof data.body !== "object") {
