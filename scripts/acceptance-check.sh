...
# ---------- 2. Bundle ID 不同 ----------
echo "-- 2. Bundle ID 独立 --"
SWIFT_ID=$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$SWIFT_APP/Contents/Info.plist" 2>/dev/null || echo "")
ELEC_ID=""
[ -n "$ELECTRON_APP" ] && ELEC_ID=$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$ELECTRON_APP/Contents/Info.plist" 2>/dev/null || echo "")
if [ -n "$SWIFT_ID" ] && [ -n "$ELEC_ID" ] && [ "$SWIFT_ID" != "$ELEC_ID" ]; then
  ok "Bundle ID 不同: $SWIFT_ID / $ELEC_ID"
else
  bad "Bundle ID 缺失或相同（swift='$SWIFT_ID' electron='$ELEC_ID'）"
fi

# ---------- 3. 产物时间戳 vs 源码（AGENTS.md 规则） ----------
echo "-- 3. 产物时间戳核对 --"
newest_swift_src=$(find "$ROOT/Sources" -name '*.swift' -type f -print0 2>/dev/null | xargs -0 stat -f '%m' 2>/dev/null | sort -rn | head -1)
[ -z "$newest_swift_src" ] && newest_swift_src=0
...
swift_bin_mtime=$(stat -f '%m' "$SWIFT_APP/Contents/MacOS/PipiUI" 2>/dev/null || echo 0)
[ "$swift_bin_mtime" -ge "$newest_swift_src" ] \
  && ok "Swift App 时间戳新鲜（bin $(stat -f '%Sm' -t '%Y-%m-%d %H:%M:%S' "$SWIFT_APP/Contents/MacOS/PipiUI")）" \
  || warn "Swift App 二进制早于最新 Sources 修改 → 需 ./make-app.sh 重新打包后再验收"
...
# ---------- 6. 已知缺口提示（不判失败） ----------
echo "-- 6. 已知缺口（人工核对，不判失败）--"
grep -q "LeaseManager" "$ROOT/Sources"/PipiUI/*.swift 2>/dev/null \
  && ok "Swift 侧已有租约实现" || warn "Swift 侧无租约实现 → 验收项3（反向阻止）待 Swift 合入"
...
MANUAL_ITEMS=(
  "双 App 同开：Swift + Electron 同时运行互不干扰（checklist 项1）"
  "会话双向发现：Electron 建会话→Swift 可见；反向亦然（项2）"
  "并发写阻止：一侧运行时另一侧只读+提示'由 X 运行中'；强杀后 45s 可恢复；强制接管生效（项3，需 Swift 租约合入）"
  "同一 UI：浏览器（配对链接）与 Electron 渲染 DOM 一致（项4，需远程合入）"
  "长会话性能：10MB+ JSONL 打开不卡、滚动流畅（项5）"
  "多 Subagent：树列表实时状态/费用 + 详情日志流 + abort/resolve/merge/discard 全链路（项6）"
  "浅色/深色跟随系统切换即时生效（项7）"
  "右栏四页：Subagents/Document/Terminal/Browser 逐一可用；无浏览器能力连接优雅降级（项8）"
)
