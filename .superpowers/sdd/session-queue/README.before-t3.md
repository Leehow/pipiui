...
- **输入栏**：Enter 发送（生成中自动转为 steer 插话）、中止按钮、底部模型切换菜单（按 provider 分组）和 thinking 等级菜单；支持粘贴/拖入/选文件添加多图预览，经 RPC `images` 发给模型，用户气泡与历史会话可显示图片
...
| `Sources/PipiUI/ChatSession.swift` | 单会话状态机：事件流 → transcript、流式组装、模型/thinking/统计命令 |
