...
final class ComputerSafetyTests: XCTestCase {
    ...
    func testRuntimeBudgetAndCursorStopLaterActionsAfterCancellation() throws {
        XCTAssertLessThan(ComputerRuntimeBudget.maximumExecutionSeconds, 35)
        ...
        XCTAssertThrowsError(try ComputerRuntimeBudget.validate(
            Array(repeating: action(.wait, duration: 10), count: 3)
        ))
    ...
    func testWriteActionsRequireNoApproval() {
    ...
    func testExpandedTerminalPolicyRequiresConfirmationUntilAllowed() {
        let terminals = [
        ...
        for (bundleID, name) in terminals {
            ...
            XCTAssertEqual(
                decision,
                .needsConfirmation,
                "\(name) must ignore legacy bundle allows and require exact code identity"
            )
            XCTAssertEqual(
