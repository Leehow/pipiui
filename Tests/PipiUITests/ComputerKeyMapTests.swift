...
final class ComputerKeyMapTests: XCTestCase {
    ...
    func testInputValidationBlocksAppSwitchAndDestructiveShortcuts() {
        ...
        for keys in [["CMD", "TAB"], ["CMD", "SPACE"], ["CMD", "Q"], ["CMD", "DELETE"]] {
            ...
            XCTAssertThrowsError(try ComputerInputSynth.shared.validate(
                actions: [action],
                imageSize: size,
                displayBounds: bounds
            ), "expected blocked shortcut \(keys)")
        }
    }
}
