...
final class ToolSkillSettingsTests: XCTestCase {
    ...
    func testReservedDesktopToolsMigrateOutOfGenericDenylistAndJSON() throws {
        let (name, suite) = tempSuite()
        defer { suite.removePersistentDomain(forName: name) }
        suite.set(
            ["computer", "open_application", "bash"],
            forKey: ToolSkillSettings.disabledToolsKey
        )
        suite.set(
            ["brainstorming"],
            forKey: ToolSkillSettings.disabledSkillsKey
        )

        XCTAssertEqual(
            ToolSkillSettings.disabledTools(defaults: suite),
            ["bash"]
        )
        XCTAssertEqual(
            suite.stringArray(forKey: ToolSkillSettings.disabledToolsKey),
            ["bash"]
        )
        XCTAssertTrue(
            ToolSkillSettings.isToolEnabled("computer", defaults: suite)
        )
        XCTAssertTrue(
            ToolSkillSettings.isToolEnabled(
                "open_application",
                defaults: suite
            )
        )

        ToolSkillSettings.setToolEnabled(
            false,
            name: "computer",
            defaults: suite
        )
        ...
        XCTAssertEqual(
            ToolSkillSettings.excludeToolNames(defaults: suite),
            ["bash"]
        )
        XCTAssertEqual(
            ToolSkillSettings.excludeToolsCLIArgs(defaults: suite),
            ["--exclude-tools", "bash"]
        )

        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "tool-skill-settings-\(UUID().uuidString).json"
            )
        defer { try? FileManager.default.removeItem(at: url) }
        ToolSkillSettings.syncJSONFile(defaults: suite, to: url)
        let object = try XCTUnwrap(
            JSONSerialization.jsonObject(
                with: Data(contentsOf: url)
            ) as? [String: Any]
        )
        XCTAssertEqual(object["disabledTools"] as? [String], ["bash"])
        XCTAssertEqual(
            object["disabledSkills"] as? [String],
            ["brainstorming"]
        )
    }
