...
final class OverlayDismissTests: XCTestCase {
    ...
    /// Contract: the remaining sheets (session/project rename, remote
    /// connection QR, finished non-text group incl. image preview, Add Model
    /// child sheet) must keep their `.dismissOnOutsideClick` wiring. Fails if
    /// production wiring is deleted. Close semantics: `sheet(item:)` nils the
    /// item; `sheet(isPresented:)` flips the bool to false.
    func testRemainingSheetsOwnDismissModifier() throws {
        let root = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let sidebar = try String(contentsOf:
            root.appendingPathComponent("Sources/PipiUI/Views/SidebarView.swift"))
        let chatDetail = try String(contentsOf:
            root.appendingPathComponent("Sources/PipiUI/Views/ChatDetailView.swift"))
        let settings = try String(contentsOf:
            root.appendingPathComponent("Sources/PipiUI/Views/SettingsSheet.swift"))

        // SidebarView: session rename sheet (sheet(item: $renameTarget)).
        XCTAssertTrue(sidebar.contains(".sheet(item: $renameTarget)"))
        XCTAssertTrue(sidebar.contains(".dismissOnOutsideClick { renameTarget = nil }"))
        // SidebarView: project rename sheet (sheet(item: $projectRenameTarget)).
        XCTAssertTrue(sidebar.contains(".sheet(item: $projectRenameTarget)"))
        XCTAssertTrue(sidebar.contains(".dismissOnOutsideClick { projectRenameTarget = nil }"))
        // SidebarView: remote connection / QR sheet.
        XCTAssertTrue(sidebar.contains(".sheet(isPresented: $showRemoteConnection)"))
        XCTAssertTrue(sidebar.contains(".dismissOnOutsideClick { showRemoteConnection = false }"))
        // ChatDetailView: finished non-text group sheet (image preview etc.).
        XCTAssertTrue(chatDetail.contains(".sheet(item: $finishedGroupPresentation)"))
        XCTAssertTrue(chatDetail.contains(".dismissOnOutsideClick { finishedGroupPresentation = nil }"))
        // SettingsSheet: Add Model child sheet (nested on the settings sheet).
        XCTAssertTrue(settings.contains(".sheet(isPresented: $showAddSheet)"))
        XCTAssertTrue(settings.contains(".dismissOnOutsideClick { showAddSheet = false }"))
    }
}
